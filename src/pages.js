App._Pages = function(window, document, Clickable, Scrollable, App, Utils,
	...
	function fixContentHeight(page) {
		...
		var bottombar = page.querySelector('.app-bottombar');
		if (!content) {
			return;
		}
		if (!topbar) {
		...
		if (!bottombar) {
			var topbarStyles = document.defaultView.getComputedStyle(topbar,
					null), topbarHeight = Utils.os.android ? 56 : 44;
			if (statusBarEnabled) {
			...
			var bottomBarStyles = document.defaultView.getComputedStyle(
					bottombar, null), bottombarHeight = Utils.os.android ? 56
					: 44;

			if (bottomBarStyles.height) {
				bottombarHeight = (parseInt(bottomBarStyles.height) || 0);
				if ((bottomBarStyles.boxSizing || bottomBarStyles.webkitBoxSizing) !== 'border-box') {
					bottombarHeight += (parseInt(bottomBarStyles.paddingBottom) || 0)
							+ (parseInt(bottomBarStyles.paddingTop) || 0);
					bottombarHeight += (parseInt(bottomBarStyles.borderBottomWidth) || 0)
							+ (parseInt(bottomBarStyles.borderTopWidth) || 0);
				}
			}
			content.style.height = (height - (topbarHeight + bottombarHeight +5 )) + 'px';
		}
